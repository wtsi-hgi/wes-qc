...
* `training_sets` - set of true-positive varisitons to train random forest model.
...
* Install pytest in your environment `uv pip install pytes`
...
**Note:** this folder is also a good example for different metadata files and theis formats.
...
* In the `general` section
  * `data_root` and `dataset_name` specifying the name of your dataset and path to it.
    By default, `data_root variable` contains reference to the dataset name, but you can use filex path if you want.
  ...
  * `onekg_resource_dir`: The place for the 1000-Genome VCFs.
    By default, it is `resources/mini_1000G` fielder under your data root
...
Put the config file and put the path to the directory
that you created for your pre-QC VCFs under the `step1 -> gatk_vcf_indir`
data entry:
...
For examples, review the `tests/test_data/metadata` folder,
created on the [Obtain resource files](#obtain-resource-files) step.

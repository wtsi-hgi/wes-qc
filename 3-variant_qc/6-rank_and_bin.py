...
def create_binned_data_initial(ht: hl.Table, bin_tmp_htfile: str, truth_htfile: str, n_bins: int) -> hl.Table:
    ...
    return (
        ...
        .aggregate(
            ...
            n_trans_common=hl.agg.filter(ht.gmonad_af >= 0.1, hl.agg.sum(ht.family_stats.tdt[0].t)),
            n_untrans_common=hl.agg.filter(ht.gmonad_af >= 0.1, hl.agg.sum(ht.family_stats.tdt[0].u)),

...
#adapted to split C>A SNPs, non C>A SNPs and indels
import hail as hl
import pyspark
...
def create_rf_ht(mtfile: str, truthset_file: str, trio_stats_file: str, allele_data_file: str, allele_counts_file: str, inbreeding_file: str, htfile_rf_all_cols: str, , htfile_rf_var_type_all_cols_CA: str, htfile_rf_var_type_all_cols_nonCA: str, htfile_rf_var_type_all_cols_indels: str):
    ...
    param str htfile_rf_var_type_all_cols_CA: Output file for RF hail table by variant type, C>A SNPs
    param str htfile_rf_var_type_all_cols_nonCA: Output file for RF hail table by variant type, non-C>A SNPs
    param str htfile_rf_var_type_all_cols_indels: Output file for RF hail table by variant type, indels
    '''
    n_partitions = 200
    ...
    ht_indels.write(htfile_rf_var_type_all_cols_indels, overwrite = True)
...
def main():
   ...
   #mtfile = mtdir + "mt_pops_QC_filters_sequencing_location_and_superpop_sanger_only_after_sample_qc.mt"
    ...
    htfile_rf_var_type_all_cols_CA = mtdir + "ht_for_RF_by_variant_type_all_cols_CA.ht"
    htfile_rf_var_type_all_cols_nonCA = mtdir + "ht_for_RF_by_variant_type_all_cols_nonCA.ht"
    htfile_rf_var_type_all_cols_indels = mtdir + "ht_for_RF_by_variant_type_all_cols_indels.ht"

    create_rf_ht(mtfile, truthset_file, trio_stats_file, allele_data_file, allele_counts_file, inbreeding_file, htfile_rf_all_cols, htfile_rf_var_type_all_cols_CA, htfile_rf_var_type_all_cols_nonCA, htfile_rf_var_type_all_cols_indels)

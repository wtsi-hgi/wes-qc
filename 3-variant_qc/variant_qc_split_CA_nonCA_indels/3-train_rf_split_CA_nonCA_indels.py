...
#modified to create separate RFs for C>A SNPs, non-C>A SNPs and indels
import hail as hl
import pyspark
import uuid
import json
...
def main():
    ...
    input_files = {
        'CA': mtdir + "ht_for_RF_by_variant_type_all_cols_CA.ht",
        'nonCA': mtdir + "ht_for_RF_by_variant_type_all_cols_nonCA.ht",
        'indels': mtdir + "ht_for_RF_by_variant_type_all_cols_indels.ht",
    }

    for vartype in input_files.keys():
        # hash for new RF
        run_hash = str(uuid.uuid4())[:8]
        rf_runs = get_rf_runs(rf_dir)
        while run_hash in rf_runs:
            run_hash = str(uuid.uuid4())[:8]
        print("Variant type:" + vartype + " runhash:" + run_hash)    

        # train RF
        input_ht_file = input_files[vartype]
        input_ht = hl.read_table(input_ht_file)
        runs_json = rf_dir + "/" + run_hash + "/rf_runs.json"

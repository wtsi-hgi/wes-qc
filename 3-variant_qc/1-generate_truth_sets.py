...
def split_multi_and_var_qc(mtfile: str, varqc_mtfile: str, varqc_mtfile_split: str) -> None:
    ...
    mt = annotate_adj(mt)
    print(f"=== Writing annotated adj mt file: {varqc_mtfile}")
    mt.write(varqc_mtfile, overwrite=True)

    mt = hl.split_multi_hts(mt)
    tmp_mt = varqc_mtfile_split + "_tmp"
    print("=== Writing split mt (temporary)")
    mt.checkpoint(tmp_mt, overwrite=True)

    print("Applying Hail variant QC to the split mt")
    mt = hl.variant_qc(mt)
    ...
    print(f"=== Writing split mt: {varqc_mtfile_split}")
...
def trio_family_dnm_annotation(
    ...
    print("=== Generating trios varqc stats")
...
def generate_ac(mt: hl.MatrixTable, fam_file: str) -> hl.Table:
    ...
    if fam_file != "":  # Looking for unrelated samples in the FAM file
...
def get_options() -> Any:
    ...
    parser.add_argument("--truth-set", help="create truth set HT", action="store_true")
...
def main() -> None:
    ...
    # get truth set ht
    truth_ht_file = os.path.join(resourcedir, "truthset_table.ht")
    if args.truth_set:
        # TODO: move to the resources preparation step
        omni = os.path.join(training_sets_dir, "1000G_omni2.5.hg38.ht")
        mills = os.path.join(training_sets_dir, "Mills_and_1000G_gold_standard.indels.hg38.ht")
        thousand_genomes = os.path.join(training_sets_dir, "1000G_phase1.snps.high_confidence.hg38.ht")
        hapmap = os.path.join(training_sets_dir, "hapmap_3.3.hg38.ht")
        get_truth_ht(
            "file://" + omni,
            "file://" + mills,
    ...
    # add hail variant QC
    mtfile = os.path.join(mtdir, "mt_pops_QC_filters_after_sample_qc.mt")
    varqc_mtfile = os.path.join(mtdir, "mt_varqc.mt")
    varqc_mtfile_split = os.path.join(mtdir, "mt_varqc_splitmulti.mt")
    if args.split_qc or args.all:  # TODO: move to the resource generation step
        split_multi_and_var_qc("file://" + mtfile, "file://" + varqc_mtfile, "file://" + varqc_mtfile_split)

    pedfile = os.path.join(annot_dir, ped_file_name) if ped_file_name != "" else ""
    ...
    fam_stats_mtfile = os.path.join(mtdir, "family_stats.mt")
    fam_stats_gnomad_mtfile = os.path.join(mtdir, "family_stats_gnomad.mt")
    ...
    if args.trios_stats or args.all:
        # TODO: Make a separate optional step. No sense in running this code without trios
        if pedfile != "":
            trio_family_dnm_annotation(
                "file://" + varqc_mtfile_split,
                "file://" + pedfile,
                "file://" + trio_mtfile,
                "file://" + trio_stats_htfile,
                "file://" + fam_stats_htfile,
                "file://" + fam_stats_mtfile,
                "file://" + fam_stats_gnomad_mtfile,
                "file://" + gnomad_htfile,
                "file://" + dnm_htfile,
            )
        else:
            print("=== !!! No trios data available for the cohort - skipping trios and family statistics !!!===")

    if args.inbreeding or args.all:
        # create inbreeding ht
        inbreeding_htfile = os.path.join(mtdir, "inbreeding.ht")
        qc_ac_htfile = os.path.join(mtdir, "qc_ac.ht")
        allele_data_htfile = os.path.join(mtdir, "allele_data.ht")
        create_inbreeding_ht_with_ac_and_allele_data(
            "file://" + varqc_mtfile,
            ("file://" + pedfile) if pedfile != "" else "",
            "file://" + inbreeding_htfile,
            "file://" + qc_ac_htfile,
            "file://" + allele_data_htfile,
        )

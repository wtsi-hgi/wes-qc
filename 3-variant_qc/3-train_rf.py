...
def get_options():
    ...
    parser.add_argument("--manual_runhash", 
...
def main():
    ...
    if args.manual_runhash:
        print(f'Using provided run ID: {args.manual_runhash}')
        run_hash = args.manual_runhash
    else:
        # hash for new RF
    ...
    ht_result = ht_result.checkpoint(get_rf(path_spark(rf_dir), data="training", run_hash=run_hash).path, overwrite=True)
    ...
    save_model(
            rf_model, get_rf(path_spark(rf_dir), data="model", run_hash=run_hash), overwrite=True)

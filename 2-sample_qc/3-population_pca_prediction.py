...
def annotate_and_filter(merged_mt_file: str, resourcedir: str, filtered_mt_file: str, pops_file: str):
    '''
...
def main():
    ...
    if args.filter or args.run:
        pops_file = inputs['kg_pop']
        annotate_and_filter(merged_mt_file, resourcedir, filtered_mt_file, pops_file)
...
#        run_pca(filtered_mt_file, pca_scores_file, pca_loadings_file, pca_evals_file)
    ...
    if args.pca or args.run:
        run_pca(filtered_mt_file, pca_scores_file, pca_1kg_scores_file, pca_1kg_loadings_file, pca_1kg_evals_file)

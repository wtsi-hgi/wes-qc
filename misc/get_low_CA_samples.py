#filter the ALSPAC data to just samples with fraction CA < 0.1
#the throey is that the high CA samples are creating excessive FP variants in variant QC
#using a CA filter to test this
...
def main():
    ...
    spectra_ht = spectra_ht.filter(spectra_ht.CA < 0.1)#filter to only low CA samples
    #load in full MT

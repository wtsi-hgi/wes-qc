...
def main() -> None:
    ...
    kg_unprocessed = os.path.join(mtdir, "kg_unprocessed.mt")
    if args.kg_to_mt:
        kgdir = inputs["kg_dir"]
        create_1kg_mt("file://" + os.path.join(resourcedir, kgdir), "file://" + kg_unprocessed)

    pruned_kg_file = os.path.join(mtdir, "kg_pruned_ld.mt")
    ...
    if args.kg_filter_and_prune:
        ...
        kg_mt_filtered = filtering.filter_matrix_for_ldprune(kg_mt, "file://" + long_range_ld_file)
        # prunning of the linked Variants

"""
Apply different combinations of hard filters to the cohost
and calculate statistics: percentage of True Positive variants, False-Positive variants,
Precision, recall and T/U ratio

The code works, but it is not optimal and requires rethinking, optimization and refactoring
"""
                ...
                for ab in ab_vals:
                    ...
                    for call_rate in missingness_vals:
                        ...
                        if not os.path.exists(json_dump_file):
                            ...
                            mt_tp_tmp, mt_fp_tmp, mt_syn_tmp, mt_prec_recall_tmp = filter_mts(
                                mt_hard, mtdir=mtdir, giab_sample=filters.giab_sample
                            )
                            var_counts = count_tp_fp_t_u(
                                mt_tp_tmp, mt_fp_tmp, mt_syn_tmp, mt_prec_recall_tmp, ht_giab, pedigree, var_type, mtdir
                            )
...
def merge_json_stats(json_files: List[str], var_type: str, final_json_file: str) -> None:
    results: ResultsDict = {var_type: {}}
...
def count_tp_fp_t_u(
    ...
    if var_type == "snv":
        ...
        prec, recall, prec_frameshift, recall_frameshift, prec_inframe, recall_inframe = get_prec_recall_indel(
            ht_prec_recall, ht_giab, mtdir
        )
        results["prec"] = prec
        results["recall"] = recall
...
def get_prec_recall_snv(ht_prec_recall: hl.Table, ht_giab: hl.Table, mtdir: str) -> tuple[float, float]:
    """
...
def get_prec_recall_indel(
    ht_prec_recall: hl.Table, ht_giab: hl.Table, mtdir: str
...
def get_trans_untrans(mt: hl.MatrixTable, pedigree: hl.Pedigree, mtdir: str) -> float:
    ...
    ratio: float = trans / untrans if untrans > 0 else 0.0

    return ratio


def count_tp_fp(mt_tp: hl.MatrixTable, mt_fp: hl.MatrixTable) -> tuple[int, int]:
    """
...
def filter_mts(
    mt: hl.MatrixTable, mtdir: str, giab_sample: str, calculate_syn_pr: bool = True
) -> tuple[hl.MatrixTable, hl.MatrixTable, Optional[hl.MatrixTable], Optional[hl.MatrixTable]]:
...
def print_results(results: ResultsDict, outfile: str, vartype: str) -> None:
    """

# counts of variants of each consequence and fraction ca per sample with range of filters
import hail as hl
import pyspark
import argparse
import pandas as pd
...
def main():
    ...
    relaxed_mt = mt.filter_entries(mt.relaxed_filters == 'Pass')
    medium_mt = mt.filter_entries(mt.medium_filters == 'Pass')
    stringent_mt = mt.filter_entries(mt.stringent_filters == 'Pass')

    cqfile_relaxed = plot_dir + "/variant_counts_per_cq_post_hard_filters_relaxed.txt"
    cafile_relaxed = plot_dir + "/frac_ca_per_sample_post_hard_filters_relaxed.txt"
    cqfile_medium = plot_dir + "/variant_counts_per_cq_post_hard_filters_medium.txt"
    cafile_medium = plot_dir + "/frac_ca_per_sample_post_hard_filters_medium.txt"
    cqfile_stringent = plot_dir + "/variant_counts_per_cq_post_hard_filters_stringent.txt"
    cafile_stringent = plot_dir + "/frac_ca_per_sample_post_hard_filters_stringent.txt"

    get_counts_per_cq(relaxed_mt, cqfile_relaxed)
    get_ca_fractions(relaxed_mt, cafile_relaxed)
    get_counts_per_cq(medium_mt, cqfile_medium)
    get_ca_fractions(medium_mt, cafile_medium)
    get_counts_per_cq(stringent_mt, cqfile_stringent)
    get_ca_fractions(stringent_mt, cafile_stringent)

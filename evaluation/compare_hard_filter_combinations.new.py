...
def filter_and_count(mt_tp: hl.MatrixTable, mt_fp: hl.MatrixTable, mt_syn: hl.MatrixTable, mt_prec_recall: hl.Table, ht_giab: hl.Table, plot_dir: str, pedfile: str, mtdir: str) -> dict:
    ...
    dp_vals = [5, 10]
    ab_vals = [0.2, 0.3]
    tmpf='/lustre/scratch126/teams/hgi/vo3_bib/qc/evaluation_tmp_res.txt'
    tmp_out=open(tmpf, 'w')
    for bin in snp_bins:
        ...
        for dp in dp_vals:
            ...
            for gq in gq_vals:
                ...
                for ab in ab_vals:
                    ...
                    tmp_out.write('snv\t'+str(filter_name)+'\t'+str(snp_counts)+'\n')
        ...
        for dp in dp_vals:
            ...
            for gq in gq_vals:
                ...
                for ab in ab_vals:
                    ...
                    tmp_out.write('indel\t'+str(filter_name)+'\t'+str(indel_counts)+'\n')
...
def filter_mts(mt: hl.MatrixTable, mtdir: str) -> tuple:
    ...
    sample = 'EGAN00004265526'#wrong-'EGAN00004410752'#GIAB sample
...
def main():
    ...
    #tmphtg = mtdir + "tmphtgx2.ht"
    #giab_ht = hl.read_table(tmphtg)

    rf_htfile = rf_dir + "b85f1884" + "/_gnomad_score_binning_tmp.ht"
    ...
    #tmpmts = mtdir + "tmpsx.mt"
    #tmpmtpr = mtdir + "tmpprx.mt"
    #mt_tp = hl.read_matrix_table(tmpmtt)
    #mt_fp = hl.read_matrix_table(tmpmtf)
    #mt_syn = hl.read_matrix_table(tmpmts)
    #mt_prec_recall = hl.read_matrix_table(tmpmtpr)

    results = filter_and_count(mt_tp, mt_fp, mt_syn, mt_prec_recall, giab_ht, plot_dir, pedfile, mtdir)

    outfile_snv = plot_dir + "/genotype_hard_filter_comparison_snv_fewer_combs.txt"
    outfile_indel = plot_dir + "/genotype_hard_filter_comparison_indel_fewer_combs.txt"
